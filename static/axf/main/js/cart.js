$(function () {
    $(".confirm").click(function () {
        ...
        $.getJSON('/axf/changecartstate/',{'cartid':cartid},function (data) {
            ...
            if(data['status'] === 200){
                ...
                if(data['c_is_select']){
                    $confirm.find('span').find('span').addClass('glyphicon glyphicon-ok');
                }else{
                    $confirm.find('span').find('span').removeClass('glyphicon glyphicon-ok');
                }
                if (data['is_all_select']){
                    $(".all_select span span").addClass('glyphicon glyphicon-ok');
                }else {
                    $(".all_select span span").removeClass('glyphicon glyphicon-ok');
                }
                if (data['is_all_select']){
                    $('.all_select span span').addClass('glyphicon glyphicon-ok');
                }else{
    ...
    $('.all_select').click(function () {
        ...
        $('.confirm').each(function () {
            ...
            if($confirm.find('span').find('span').attr('class')){
        ...
        if (unselect_list.length > 0){
            $.getJSON('/axf/allselect/',{'cart_list':unselect_list.join('#')},function (data) {
                ...
                if(data['status'] === 200){
                    $('.confirm').find('span').find('span').addClass('glyphicon glyphicon-ok');
                    $all_select.find('span').find('span').addClass('glyphicon glyphicon-ok');
                    $('#total_price').html(data['total_price']);
                }
            })
        }else{
            if (select_list.length > 0){
                ...
                if(data['status'] === 200){
                    $('.confirm').find('span').find('span').removeClass('glyphicon glyphicon-ok');
                    $all_select.find('span').find('span').removeClass('glyphicon glyphicon-ok');
                    $('#total_price').html(data['total_price']);
                }
            })
            }
